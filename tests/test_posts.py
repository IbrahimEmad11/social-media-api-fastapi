...
@pytest.mark.parametrize("title, content, published", [
    ("Nasser el desouki", "Essam el Nemr halfoot", True),
    ("Dubai", "Ana atwl mn burj khalifa", False),
    ("Food", "nefsy a3oom f chocolate", True),
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        "/posts/", json={"title": "blabla", "content": "lmao"})

    created_post = schemas.PostResponse(**res.json())
    assert res.status_code == 201
    assert created_post.title == "blabla"
    assert created_post.content == "lmao"
